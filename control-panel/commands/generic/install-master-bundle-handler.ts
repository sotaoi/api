...
class InstallMasterBundleHandler extends TaskHandler {
  public getFormId = async (): Promise<string> => 'install-master-bundle-task';

  // public async handle(task: TaskCommand): Promise<TaskResult> {
  public async handle(task: TaskCommand): Promise<any> {
    // try {
    //   const BundleJson = Helper.getBundleJson();
    //   if (BundleJson.installed) {
    //     throw new Error('Bundle is already installed');
    //   }
    //   await InstallBundleHelper(task, true);
    //   BundleJson.installed = true;
    //   BundleJson.master = true;
    //   fs.writeFileSync(require.resolve('@sotaoi/omni/bundle.json'), JSON.stringify(BundleJson, null, 2));
...
export { InstallMasterBundleHandler };
